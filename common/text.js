...
{
  let reg = new RegExp(key, "gmi"), ps = name.split(".");
  let vs = list.map(i => propertyValue(i, ps).toString()); //console.log(vs); return;
  let rs = vs.map((i, index) =>
  {
    let matchs = i.match(reg); if (matchs) return { index: index, count: matchs.length }; else return null;
  }).filter(i => i);
  rs = Enumerable.from(rs).orderByDescending(r => r.count).toArray();
  return rs.map(r => list[r.index]);
}
exports.search = search;

function propertyValue(obj, ps)
{
  //console.log(obj, ps);
  let c = ps.length; if (c == 0 || (typeof obj) != 'object') return '';
  let r = obj; for (let i = 0; i < c; i++) { r = r[ps[i]]; if (!r) return ''; } console.log(r); return r;
}
